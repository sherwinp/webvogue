...
public class EncodePurchaseTransaction extends com.d_project.qrcode.web.QRCodeServlet {
	...
	public String EncodeData(HttpServletRequest request) {
		...
		return String.format("%s://%s:%s%s/%s/%s/%s", request.getScheme(), request.getServerName(), request.getServerPort(), request.getContextPath(), "api/pay/payMent", data, request.getSession(false) != null ? request.getSession(false).getId() : "");
	}
